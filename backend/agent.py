...
class ReportAgent:
    ...
    async def stream_report_generation(self, combined_blueprint_text: str, user_prompt: str):
        ...
        system_prompt = (
            ...
            "2.  **TARGET LENGTH & DETAIL:** Your output MUST be detailed and comprehensive, aiming for a length of approximately **7-8 pages** when rendered as a PDF. Match the detail level found in the 'Markt-Report Italien Biomasse_GPT-5_v1.1.pdf' example (multi-paragraph sections, specific data, nuanced analysis), adapting content for ITALY. Short, superficial answers are a failure."
            ...
            "    * You MUST generate plausible, realistic data relevant to ITALY for ALL requested columns."
            ...
            "\n### REQUIRED CONTENT DETAIL (Per Section for ITALY - Be Detailed, Use Latest Data, Write in German) ###\n"
            ...
            "   - **### Energiemix**: Provide recent % breakdown for ALL sources in ITALY (Gas, Öl, Erneuerbare - spezifizieren Sie Wind, Solar, Biomasse, Wasser, Kohle/Braunkohle, Status Kernenergieausstieg, Importe). Discuss current trends (Fortschritt Energiewende, Kohleausstiegspläne) and cite specific national energy targets (EEG, Klimaschutzgesetz). Mention current infrastructure status (Gaspipelines wie Nord Stream-Implikationen, Netzausbaubedarf).\n"
            
            "**## C. Biomasse & Abfallwirtschaft**:\n"
            "   - **### Marktspezifika**: Include recent data for ITALY's wood pellet market (Produktion, Verbrauch, Preistrends, Import/Export) and for Holzchips (Verwendung in KWK-Anlagen, Hauptquellen). Identify major biomass associations (e.g., DEPV, FNR).\n"
            ...
            "   - **### Abfallwirtschaft**: Include recent recycling rates (kommunal, Verpackungen - Grüner Punkt), Deponieverbote (Kreislaufwirtschaftsgesetz), specifics on regulations like Gewerbeabfallverordnung in ITALY.\n"
            
            "**## D. Transport & Logistik**:\n"
            "   - **### Logistikinfrastruktur**: Detail current status of ports (Hamburg, Bremen/Bremerhaven, Rostock), rail network (DB Cargo), key waterways (Rhine, Elbe), inland hubs relevant to biomass in ITALY.\n"
            "   - **### Top-5 Logistikdienstleister (Separate Tabellen für LKW, Bahn, Schiff)**: Include current Name, Sitz, Volumen/Umsatz, Kontakt specific to ITALY (e.g., major freight forwarders, rail operators).\n"
            ...
            "   - **### Markteintritt**: Concrete strategy recommendations (e.g., office location in ITALY, partnerships with Stadtwerke or agricultural cooperatives).\n"
            
            "**## G. Expansionsstrategien & Wachstumshebel**:\n"
            "   - **### Konkrete Strategien**: Make expansion blueprints specific to ITALY with suggested actions/locations (e.g., focus on regions with high forestry or CHP plant density).\n"
            
            "**## H. Fazit & Handlungsempfehlungen**:\n" 
            "   - **### Priorisierte Maßnahmen**: Provide specific, prioritized, actionable recommendations for ITALY based on current market conditions (target regions, compliance needs like EEG/GEG, partnership opportunities).\n"

            "Generate the full, detailed, professional, German-only report for ITALY now, using the latest available data (2024-2025 preferred) and aiming for approximately 7-8 pages."
        )
